...
# -*- coding: utf-8 -*-
"""
Context Weaver avec Neural Graph Learner (MLP + Transformers)

✅ VERSION SIMPLIFIÉE: Uniquement Neural (pas de statistique/hybride)
"""
...
class ContextWeaverWithNeuralValidation:
    """
    Context Weaver étendu avec validation par Neural Graph Learner
    
    Pipeline simplifié:
    1. Classification OSS
    2. Normalisation
    3. TaxonomyPipeline (Retrieval + Validation)
    4. Validation neuronale (MLP + Transformers)
    5. Context Weaver final
    """
    ...
    def __init__(
        ...
        taxonomy_pipeline,
        context_weaver,
        neural_learner_path: Optional[Path] = None,
        neural_config = None
    ):
        """
        Initialise le pipeline avec Neural Learner
        
        Args:
            oss_classifier: Classifier OSS 20B
            normalizer: Normalizer taxonomique
            taxonomy_pipeline: Pipeline taxonomy complet
            context_weaver: Assembleur de contexte
            neural_learner_path: Chemin du modèle neural
            neural_config: NeuralConfig (optionnel)
        """
        self.oss_classifier = oss_classifier
        self.normalizer = normalizer
        self.taxonomy_pipeline = taxonomy_pipeline
        self.context_weaver = context_weaver
        
        # === Neural Graph Learner ===
        from context_weaver.learner.neural import (
            NeuralGraphLearnerWrapper,
            NeuralConfig
        )
        
        model_path = neural_learner_path or Path("./data/neural_graph_model.pt")
        config = neural_config or NeuralConfig()
        
        self.neural_learner = NeuralGraphLearnerWrapper(
            model_path=model_path,
            config=config
        )
        
        logger.info("✅ Context Weaver avec Neural Validation initialisé")
    ...
    def process(
        ...
        """
        Pipeline complet avec validation neuronale
        ...
        """
        logger.info("=" * 80)
        logger.info("🔄 CONTEXT WEAVER AVEC NEURAL VALIDATION")
        logger.info("=" * 80)
        ...
        # Ajouter structure hint du neural learner
        try:
            structure_hypothesis = self.neural_learner.predict_structure(
                context={
                    'domain': classification.domain,
        ...
        # === ÉTAPE 4: Validation Neural ===
        logger.info("\n4️⃣ Validation Neural Learner...")
        
        validated_results = []
        
        if taxonomy_output.retrieval_result:
            validated_results = self._validate_candidates_neural(
    ...
    def _validate_candidates_neural(
        self,
        candidates: List,
        classification,
        normalized
        ...
        """
        Valide et score les candidats avec Neural Learner
        ...
        Returns:
            Candidats enrichis avec validation neuronale
        """
        logger.info(f"   🔍 Validation neuronale de {len(candidates)} structures...")
        ...
        for candidate in candidates:
            # Convertir TaxonCandidate en structure
            structure = {
            ...
            # Valider avec Neural Learner
            try:
                validation = self.neural_learner.validate_structure(structure, context)
                
                # Enrichir metadata
                if not hasattr(candidate, 'metadata') or candidate.metadata is None:
                    candidate.metadata = {}
                
                candidate.metadata['neural_validation'] = {
        ...
        if validated_candidates:
            high = sum(1 for c in validated_candidates if c.metadata.get('neural_validation', {}).get('category') == 'HIGH')
            medium = sum(1 for c in validated_candidates if c.metadata.get('neural_validation', {}).get('category') == 'MEDIUM')
            low = sum(1 for c in validated_candidates if c.metadata.get('neural_validation', {}).get('category') == 'LOW')
    ...
    def _get_validation_summary(self, validated_candidates: List) -> Dict[str, Any]:
        ...
        categories = [
            c.metadata.get('neural_validation', {}).get('category', 'MEDIUM')
            for c in validated_candidates
        ]
        
        scores = [
            c.metadata.get('neural_validation', {}).get('score', 0.5)
            for c in validated_candidates
        ]
        
        confidences = [
            c.metadata.get('neural_validation', {}).get('confidence', 0.0)
            for c in validated_candidates
        ]
    ...
    def train_neural_learner(self, graph_data: Dict[str, Any]):
        """
        Entraîne le Neural Learner avec nouvelles données
        
        Args:
            graph_data: Structure du graphe {nodes: [...], edges: [...]}
        """
        logger.info("🎓 Entraînement du Neural Learner...")
        self.neural_learner.train_from_graph(graph_data)
        logger.info("✅ Entraînement terminé")
    
    def get_learner_metrics(self) -> Dict[str, Any]:
        """Retourne les métriques du Neural Learner"""
        return self.neural_learner.get_learning_metrics()
    ...
    def provide_feedback(
        ...
        """
        Fournit un feedback pour online learning
        
        Args:
            structure_id: ID de la structure
            was_correct: Validation correcte ou non
            context: Contexte utilisateur
        """
        self.neural_learner.provide_feedback(
            structure_id,
            was_correct,
            {'context': context}
        )
...
def example_usage():
    ...
    print("\n" + "=" * 80)
    print("📘 EXEMPLE: Context Weaver avec Neural Validation")
    print("=" * 80 + "\n")
    ...
    from context_weaver.learner.neural import NeuralConfig
    ...
    # === CRÉER LE PIPELINE AVEC NEURAL VALIDATION ===
    pipeline = ContextWeaverWithNeuralValidation(
        oss_classifier=oss_classifier,
        normalizer=normalizer,
        taxonomy_pipeline=taxonomy_pipeline,
        context_weaver=context_weaver,
        neural_learner_path=Path("./data/neural_graph_model.pt"),
        neural_config=neural_config
    )
    
    # === ENTRAÎNER (optionnel si modèle déjà entraîné) ===
    print("ℹ️  Note: Utilisez train_neural_learner.py pour entraîner le modèle")
    print("   ou décommentez la section ci-dessous pour entraîner maintenant\n")
    
    # Uncomment pour entraîner:
    # print("🎓 Phase d'entraînement...")
    # from train_neural_learner import extract_graph_from_dgraph
    ...
    # connector.close()
    # 
    # pipeline.train_neural_learner(graph_data)
    # print("\n" + "-" * 80)
    ...
    for i, result_item in enumerate(result['search_results'].results[:3], 1):
        ...
        if 'neural_validation' in result_item.metadata:
            val = result_item.metadata['neural_validation']
            print(f"     Validation: {val['category']} (score={val['score']:.3f})")
    
    # === MÉTRIQUES DU LEARNER ===
    print("\n" + "=" * 80)
    print("📈 MÉTRIQUES DU NEURAL LEARNER")
    print("=" * 80)
    
    metrics = pipeline.get_learner_metrics()
    print(f"  • Type: {metrics.get('model_type', 'neural')}")
    print(f"  • Device: {metrics.get('device', 'cpu')}")
