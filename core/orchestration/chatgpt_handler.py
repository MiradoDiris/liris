"""
Script de connexion et interaction automatique avec ChatGPT
"""
...
def interact_with_chatgpt(message):
    """Interagit automatiquement avec ChatGPT"""
    
    print("🤖 Connexion à ChatGPT")
    print("=" * 70)
    ...
    print("\n2️⃣  Navigation vers ChatGPT")
    client.navigate("https://chatgpt.com/")
    print("✅ Page ChatGPT chargée")
    time.sleep(3)
    ...
    if isinstance(elements, list):
        for elem in elements:
            ...
            # Chercher le champ de saisie (textarea ou input text)
            if not input_field:
                if elem_type == "textarea":
                    input_field = elem
                    print(f"   💬 Champ de saisie trouvé (textarea): nodeId={elem.get('nodeId')}")
                elif elem_type == "input":
                    if "message" in elem_placeholder or "prompt" in elem_placeholder:
                        input_field = elem
                        print(f"   💬 Champ de saisie trouvé (input): nodeId={elem.get('nodeId')}")
            ...
            if not send_button:
                if elem_type == "button":
                    if "send" in elem_text or "submit" in elem_text or "envoyer" in elem_text:
                        send_button = elem
...
if __name__ == "__main__":
    MESSAGE = "Explique-moi l'intelligence artificielle en 3 phrases simples"
    
    client = interact_with_chatgpt(MESSAGE)