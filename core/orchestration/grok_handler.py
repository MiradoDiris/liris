"""
Script de connexion et interaction automatique avec Grok
"""
...
def interact_with_grok(message):
    """Interagit automatiquement avec Grok"""
    
    print("🚀 Connexion à Grok")
    print("=" * 70)
    ...
    # 2. Navigate to Grok
    print("\n2️⃣  Navigation vers Grok")
    client.navigate("https://grok.com/")
    print("✅ Page Grok chargée")
    time.sleep(3)
    ...
    if isinstance(elements, list):
        for elem in elements:
            ...
            if not input_field:
                if elem_type == "textarea":
                    ...
                    if input_type in ["text", "search", ""]:
                        if "message" in elem_placeholder or "prompt" in elem_placeholder or "ask" in elem_placeholder:
                            input_field = elem
            ...
            if not send_button:
                if elem_type == "button":
                    if "send" in elem_text or "submit" in elem_text or "send" in elem_aria or "ask" in elem_text:
                        send_button = elem
...
if __name__ == "__main__":
    MESSAGE = "Raconte-moi une blague sur l'intelligence artificielle"
    
    client = interact_with_grok(MESSAGE)