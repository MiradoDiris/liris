...
from PyQt5.QtCore import Qt, pyqtSignal, QTimer # QTimer pour les mises à jour non bloquantes de l'UI
...
class ResultsDialog(QtWidgets.QDialog):
    ...
    def _populate_results(self):
        ...
        for result in self.generated_mutations:
            ...
            mutation_code = result.get('mutation_code', 'Aucun code de mutation généré.')
            ...
            view_code_button.clicked.connect(lambda checked, code=mutation_code, name=file_name: self._show_code_dialog(code, name))
            ...
            execute_code_button.clicked.connect(lambda checked, code=mutation_code, name=file_name: self._on_execute_code(code, name))
    ...
    def _on_execute_code(self, code, file_name):
        ...
        logger.info(f"Action: Exécuter le code pour '{file_name}' (Code: {code[:50]}...)")
        QtWidgets.QMessageBox.information(self, "Exécuter le Code", f"L'exécution du code pour '{file_name}' n'est pas encore implémentée.")
    ...
    def _on_execute_all(self):
        ...
        QtWidgets.QMessageBox.information(self, "Exécuter Tout", "L'exécution de tous les codes n'est pas encore implémentée.")
...
class NodeCreationWidget(QtWidgets.QWidget):
    ...
    def _init_ui(self):
        ...
        self.file_list_widget.setSelectionMode(QtWidgets.QAbstractItemView.ExtendedSelection) # Permettre la sélection multiple
        ...
        self.platforms_list_widget.setSelectionMode(QtWidgets.QAbstractItemView.MultiSelection)
        ...
        self.response_area = QtWidgets.QTextEdit() # Renommée pour refléter son rôle de log
        ...
        main_layout.addWidget(self.proceed_button, alignment=Qt.AlignCenter) # Centrer le bouton
    ...
    def _populate_project_selection(self):
        ...
        try:
            ...
            if self.project_combo.count() > 1:
                self.project_combo.setCurrentIndex(1) # Sélectionne le premier vrai projet
                # _on_project_selected sera appelé automatiquement
            else:
                self._on_project_selected(0) # Appel manuel si un seul élément (l'option par défaut)

        except Exception as e:
            print(f"Erreur lors du chargement des projets depuis la base de données: {e}")
    ...
    def _on_project_selected(self, index):
        ...
        if index == 0 or not selected_project_name or not self.conductor or not self.conductor.database:
        ...
        self.proceed_button.setEnabled(True) # Activer le bouton si un projet valide est sélectionné

        try:
            project_profile_data = self.conductor.database.get_project_profile(selected_project_name)
            if project_profile_data:
                ...
                for cluster_data in clusters_detailed:
                    ...
                    self._collect_files_from_ontology(cluster_data.get('root_labels', []), unique_categories, cluster_id)
                ...
                self.file_list_widget.addItem("Aucune donnée d'ontologie trouvée pour ce projet.")

        except Exception as e:
            print(f"Erreur lors du chargement des données d'ontologie pour le projet {selected_project_name}: {e}")
            self.file_list_widget.addItem(f"Erreur de chargement des fichiers: {e}")

    def _collect_files_from_ontology(self, nodes_list, unique_categories_set, current_cluster_id=None):
        """
        ...
        for node in nodes_list:
            ...
            node_full_path = node.get('full_path') # Récupérer le chemin complet du nœud

            if node.get('type') == 'file':
                file_name = node.get('name', 'N/A')
                file_categories = node.get('category', []) # category est une liste de chaînes
            ...
            if 'parent_labels' in node and isinstance(node['parent_labels'], list):
                self._collect_files_from_ontology(node['parent_labels'], unique_categories_set, current_cluster_id)
            
            # Récursion dans child_labels (enfants du parent)
            if 'child_labels' in node and isinstance(node['child_labels'], list):
                self._collect_files_from_ontology(node['child_labels'], unique_categories_set, current_cluster_id)
    ...
    def _on_proceed_creation(self):
        ...
        self.proceed_button.setEnabled(False) # Désactiver le bouton pendant le traitement
        QtWidgets.QApplication.processEvents() # Mettre à jour l'UI
        ...
        if not selected_file_items:
            QtWidgets.QMessageBox.warning(self, "Sélection de Fichier", "Veuillez sélectionner au moins un fichier dans la liste.")
            self.proceed_button.setEnabled(True)
            return
        ...
        if not selected_files_info:
            QtWidgets.QMessageBox.critical(self, "Erreur de Fichier", "Impossible de trouver les informations détaillées pour les fichiers sélectionnés.")
            self.proceed_button.setEnabled(True)
            return
        ...
        for i in range(self.platforms_list_widget.count()):
            ...
            if item.checkState() == Qt.Checked:
                ...
                if platform_name in self.platforms:
                    ...
                    self._debug_log(f"WARNING: La plateforme '{platform_name}' n'a pas été trouvée parmi les profils disponibles.")

        if not selected_platforms_profiles:
            QtWidgets.QMessageBox.warning(self, "Sélection de Plateforme", "Veuillez sélectionner au moins une plateforme IA à utiliser.")
            self.proceed_button.setEnabled(True)
            return
        ...
        for file_info in selected_files_info:
            ...
            if not file_path or not os.path.exists(file_path):
                self._debug_log(f"❌ Le chemin du fichier '{file_name}' est invalide ou le fichier n'existe pas: {file_path}")
...
"""
            ...
            for platform_profile in selected_platforms_profiles:
                ...
                try:
                    ...
                    prompt_field = platform_profile.get('interface_positions', {}).get('prompt_field')
                    extraction_config = platform_profile.get('extraction_config', {})
                    detection_config = platform_profile.get('detection_config', {})
                    send_keys_config = platform_profile.get('keyboard', {}).get('send_keys_config', {})

                    if not window_position or 'x' not in window_position or 'y' not in window_position:
                        self._debug_log(f"  ❌ Configuration incomplète pour {platform_name}: window_position invalide.")
                    ...
                    if not prompt_field or 'center_x' not in prompt_field or 'center_y' not in prompt_field:
                        self._debug_log(f"  ❌ Configuration incomplète pour {platform_name}: prompt_field invalide.")
                        continue # Passer à la plateforme suivante

                    # ÉTAPE 2: Clic icône fenêtre et ouverture URL
                    self._debug_log(f"  - Clic icône fenêtre et ouverture URL pour {platform_name}...")
                    try:
                        ...
                        if platform_url:
                            ...
                            result = self.conductor.browser_manager.open_url(platform_url, browser_type, new_window=False) # Use browser_type here
                            if result.get('success'):
                                time.sleep(4)  # Attendre chargement page
                                self._debug_log("URL ouverte avec succès")
                            else:
                                self._debug_log(f"⚠️ Échec ouverture URL: {result.get('error', 'Erreur inconnue')}")
                        else:
                            self._debug_log("⚠️ Aucune URL configurée")
                        
                        self._debug_log(f"  ✅ Navigateur prêt pour {platform_name}.")
                    except Exception as e:
                        self._debug_log(f"  ❌ Erreur lors de la préparation du navigateur pour {platform_name}: {e}")
                        continue

                    # ÉTAPE 3: Clic champ de saisie
                    self._debug_log(f"  - Clic sur le champ de saisie pour {platform_name}...")
                    try:
                        ...
                        self._debug_log(f"  ❌ Erreur clic champ de saisie pour {platform_name}: {e}")
                        continue

                    # ÉTAPE 4: Nettoyage champ
                    self._debug_log(f"  - Nettoyage du champ de saisie pour {platform_name}...")
                    try:
                        ...
                        self._debug_log(f"  ❌ Erreur nettoyage champ pour {platform_name}: {e}")
                        continue
                    ...
                    try:
                        ...
                        self._debug_log(f"  ❌ Erreur saisie texte pour {platform_name}: {e}")
                        continue
                    ...
                    try:
                        ...
                        self._debug_log(f"  ❌ Erreur envoi formulaire pour {platform_name}: {e}")
                        continue

                    # ÉTAPE 7: Attente réponse avec DÉTECTION UNIVERSELLE
                    self._debug_log(f"  - Attente de la fin de génération de réponse pour {platform_name}...")
                    response_completed = False
                    try:
                        response_completed = self._wait_for_ai_completion(detection_config, platform_profile, browser_type)
                        if response_completed:
                            self._debug_log(f"  ✅ Détection de fin de génération réussie pour {platform_name}.")
                        else:
                            self._debug_log(f"  ⚠️ Timeout de détection pour {platform_name}. La réponse pourrait être incomplète.")
                    except Exception as e:
                        self._debug_log(f"  ❌ Erreur lors de la détection de réponse pour {platform_name}: {e}")
                        print(traceback.format_exc())
                        
                    # ÉTAPE 8: Extraction réponse avec EXTRACTION UNIVERSELLE
                    self._debug_log(f"  - Extraction de la réponse pour {platform_name}...")
                    ai_response_text = "Aucune réponse extraite."
                    try:
                        extracted_text = self._extract_response_universal(extraction_config, platform_profile, browser_type)
                        if extracted_text:
                            ai_response_text = extracted_text
                        else:
                            ai_response_text = "Réponse extraite, mais vide."
                        self._debug_log(f"  ✅ Extraction de la réponse réussie pour {platform_name}.")
                    except Exception as e:
                        ai_response_text = f"Erreur lors de l'extraction de la réponse: {e}"
                    ...
                    generated_mutations_results.append({
                        'file_name': file_name,
                        'mutation_code': ai_response_text
                    })
    ...
    def _wait_for_ai_completion(self, detection_config, platform_profile, detected_browser_type):
        ...
        try:
            ...
            if universal_config:
                self._debug_log(f"🎯 Utilisation détection universelle pour {universal_config['platform']}")
            ...
            window_position = platform_profile.get('window_position', {})
            if window_position:
                self._debug_log(f"Focus fenêtre avant détection: ({window_position['x']}, {window_position['y']})")
                self.conductor.mouse_controller.click(window_position['x'], window_position['y'])
                time.sleep(0.2)
            
            return self._execute_detection_script(js_code, platform_profile, detected_browser_type)
    ...
    def _execute_detection_script(self, js_code, platform_profile, detected_browser_type):
        ...
        try:
            ...
            self._debug_log(f"👀 Surveillance console (max {max_wait}s, check chaque {check_interval}s)")
            ...
            while waited < max_wait: # Removed self.should_stop
                ...
                if waited % 2 == 0:
                    self._debug_log(f"⏳ Attente détection... {waited:.1f}s/{max_wait}s")
    ...
    def _extract_response_universal(self, extraction_config, platform_profile, detected_browser_type):
        ...
        try:
            ...
            window_position = platform_profile.get('window_position', {})
            if window_position:
                self._debug_log(f"Focus fenêtre avant extraction: ({window_position['x']}, {window_position['y']})")
                self.conductor.mouse_controller.click(window_position['x'], window_position['y'])
            ...
            return self._extract_response_simple_fallback(extraction_config, platform_profile, detected_browser_type)
    ...
    def _execute_extraction_script(self, js_code, detected_browser_type):
        ...
        try:
            ...
            if result:
                ...
                if not has_excluded:
                    self._debug_log(f"✅ Réponse universelle valide extraite: {len(result)} caractères")
                    return result
                else:
    ...
    def _extract_response_simple_fallback(self, extraction_config, platform_profile, detected_browser_type):
        """
