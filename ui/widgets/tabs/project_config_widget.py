...
class ProjectConfigWidget(QtWidgets.QWidget):
    ...
    def __init__(self, config_provider, conductor, parent=None):
        ...
        if not self.database:
            logger.error("Database instance not available from conductor. ProjectConfigWidget cannot function correctly.")
        ...
        try:
            ...
            logger.error(f"Erreur lors de l'initialisation de ProjectConfigWidget : {str(e)}")
    ...
    def _init_ui(self):
        ...
        details_form_layout.setAlignment(Qt.AlignHCenter | Qt.AlignTop) # Alignement horizontal au centre, vertical en haut
        ...
        details_form_layout.addRow(cluster_list_layout) # Ajout du layout des clusters au formulaire
        ...
        top_columns_layout.addLayout(left_column_layout, 3) # Facteur d'étirement de 3 pour la colonne de gauche (environ 15%)
        ...
        hierarchy_group_layout.setSpacing(10) # Espacement pour les éléments dans ce groupe
        ...
        hierarchy_layout.addWidget(hierarchy_group) # Ajoute le groupbox de hiérarchie au layout de la colonne droite
        ...
        top_columns_layout.addWidget(right_column_container, 17) # Facteur d'étirement de 17 pour la colonne de droite (environ 85%)
        ...
        self.save_button.setEnabled(False) # Désactivé jusqu'à ce qu'un projet soit sélectionné ou ajouté
        save_export_layout.addWidget(self.save_button)

        # Nouveau bouton Exporter requête Dgraph
        self.export_dgraph_query_button = QtWidgets.QPushButton("🚀 " + tr("project_config.export_dgraph_query_button"))
        self.export_dgraph_query_button.setStyleSheet(PlatformConfigStyle.get_button_style())
        ...
        save_export_layout.addStretch()
        main_vertical_layout.addLayout(save_export_layout) # Ajoute le layout des boutons au layout vertical principal
    ...
    def update_language(self):
        ...
        self.findChild(QtWidgets.QLabel, None, Qt.FindDirectChildrenOnly).setText(tr("project_config.explanation"))
        
        # Mettre à jour les titres de groupe en utilisant leur objectName pour une meilleure robustesse
        self.findChild(QtWidgets.QGroupBox, "project_selection_group").setTitle(tr("project_config.select_profile_group"))
        self.findChild(QtWidgets.QGroupBox, "details_group").setTitle(tr("project_config.details_group"))
        self.findChild(QtWidgets.QGroupBox, "hierarchy_group").setTitle(tr("project_config.hierarchy_group"))
        ...
        for label in self.findChildren(QtWidgets.QLabel):
            ...
            if label.text() == tr("project_config.project_name_label_old"): # Pour les cas où tr() retourne le même texte si la clé n'est pas trouvée
                label.setText(tr("project_config.project_name_label"))
            elif label.text() == tr("project_config.cluster_label_old"): # Ancienne clé
                label.setText(tr("project_config.cluster_label")) # Nouvelle clé pour le titre de la liste
            elif label.text() == tr("project_config.root_labels_list_label_old"):
                label.setText(tr("project_config.root_labels_list_label"))
            elif label.text() == tr("project_config.parent_labels_list_for_root_label_old"):
        ...
        self.modify_category_root_button.setText(tr("project_config.modify_category_button"))
        ...
        self.modify_category_parent_button.setText(tr("project_config.modify_category_button"))
        ...
        self.modify_category_child_button.setText(tr("project_config.modify_category_button"))
        ...
        self.export_dgraph_query_button.setText("🚀 " + tr("project_config.export_dgraph_query_button"))
    ...
    def _on_project_selected(self, index):
        ...
        if index <= 0:
            ...
            self.export_dgraph_query_button.setEnabled(False) # Désactiver à la désélection
            self._update_button_states()
            return
        ...
        self.current_project_profile_data = self.project_profiles.get(project_name, {}).copy() 
        ...
        self.export_dgraph_query_button.setEnabled(self.is_configured()) # Activer si le projet est configuré
        self._update_button_states()
    ...
    def _load_profile_into_ui(self, project_name):
        ...
        if clusters_detailed_data:
            ...
            self.cluster_list_widget.currentItemChanged.disconnect(self._on_cluster_selected)
            self.cluster_list_widget.setCurrentRow(0) # Ceci déclenchera _on_cluster_selected
            self.cluster_list_widget.currentItemChanged.connect(self._on_cluster_selected)
    ...
    def _reset_ui(self):
        ...
        self.export_dgraph_query_button.setEnabled(False) # Désactiver le bouton d'exportation
    ...
    def _update_button_states(self):
        ...
        self.edit_cluster_button.setEnabled(is_project_selected and has_cluster_selected)
        self.remove_cluster_button.setEnabled(is_project_selected and has_cluster_selected)

        self.import_folder_button.setEnabled(True) # Toujours enabled pour pouvoir importer
        self.add_project_button.setEnabled(True) # Toujours enabled pour ajouter un nouveau projet
        ...
        self.edit_root_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected)
        self.remove_root_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected)
        ...
        self.modify_category_root_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and selected_root_data and selected_root_data.get('type') == 'file')


        # Contrôles des Labels Parents (dépendent de la sélection d'un cluster et d'une racine)
        self.add_parent_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected)
        self.edit_parent_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and has_parent_selected)
        self.remove_parent_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and has_parent_selected)
        self.parent_list_widget.setEnabled(is_project_selected and has_cluster_selected and has_root_selected)
        # Modify Category for Parent
        selected_parent_data = self._get_selected_label_data("parent")
        self.modify_category_parent_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and has_parent_selected and selected_parent_data and selected_parent_data.get('type') == 'file')


        # Contrôles des Labels Enfants (dépendent de la sélection d'un cluster, d'une racine et d'un parent)
        self.add_child_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and has_parent_selected)
        self.edit_child_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and has_parent_selected and has_child_selected)
        self.remove_child_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and has_parent_selected and has_child_selected)
        self.child_list_widget.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and has_parent_selected)
        # Modify Category for Child
        selected_child_data = self._get_selected_label_data("child")
        self.modify_category_child_button.setEnabled(is_project_selected and has_cluster_selected and has_root_selected and has_parent_selected and has_child_selected and selected_child_data and selected_child_data.get('type') == 'file')

        self.save_button.setEnabled(is_project_selected) # Le bouton Save dépend uniquement de la sélection d'un projet
        self.export_dgraph_query_button.setEnabled(is_project_selected and self.is_configured()) # Activer si le projet est sélectionné et configuré
    ...
    def _on_cluster_selected(self, current, previous):
        ...
        self.current_parent_label_index = -1
        self.current_child_label_index = -1
        self.current_cluster_data = None # Réinitialiser avant de potentiellement charger
        ...
        if 0 <= self.current_cluster_index < len(clusters_detailed_data):
            self.current_cluster_data = clusters_detailed_data[self.current_cluster_index]
            logger.debug(f"Cluster sélectionné. Index: {self.current_cluster_index}, Nom: {self.current_cluster_data.get('name')}")
            ...
            for r_label in root_labels_data:
                ...
                if r_label.get('type') == 'file' and r_label.get('category') and len(r_label['category']) > 0:
            ...
            if root_labels_data:
                self.root_list_widget.setCurrentRow(0) # Cela déclenchera _on_root_label_selected
            else:
                logger.debug("Aucun label racine dans le cluster sélectionné, les listes parent/enfant sont effacées.")
        ...
        try:
            ...
            if self.current_project_name and self.current_project_profile_data:
                reply = QtWidgets.QMessageBox.question(
                    ...
                    tr("project_config.confirm_import_add_cluster_text").format(cluster_name=os.path.basename(selected_directory)), # Use base name for message
                    QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, # Yes: Add to current project, No: Create new project
                    QtWidgets.QMessageBox.Yes # Default to adding to current
                )
                if reply == QtWidgets.QMessageBox.No: # User chose to create a new project
                    new_project_name, ok = QtWidgets.QInputDialog.getText(
                        self, tr("project_config.new_project_for_import_title"),
                        tr("project_config.new_project_for_import_text").format(cluster_name=os.path.basename(selected_directory)),
                        QtWidgets.QLineEdit.Normal, os.path.basename(selected_directory)
                    )
                    ...
                    if new_project_name in self.project_profiles:
                        QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"),
                                                     tr("project_config.project_exists_msg").format(project_name=new_project_name))
                        return
            ...
            logger.debug(f"Début de peuplement de l'ontologie pour '{selected_directory}'")
            ...
            for item_name in sorted(os.listdir(selected_directory)):
                ...
                existing_cluster = next((c for c in turing_ontology['clusters_detailed'] if c['name'] == cluster_name), None)

                if existing_cluster:
                    replace_reply = QtWidgets.QMessageBox.question(
                        self,
                        tr("project_config.cluster_exists_title"),
                        tr("project_config.cluster_exists_msg").format(cluster_name=cluster_name),
                        QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No # Yes: Replace, No: Cancel
                    )
                    if replace_reply == QtWidgets.QMessageBox.No:
                        logger.info(f"Importation annulée : cluster '{cluster_name}' existant non remplacé.")
                        continue # Skip to the next item if user cancels for this cluster
                    else:
                        existing_cluster['root_labels'] = [] # Clear existing root labels
                    ...
                    turing_ontology['clusters_detailed'].append(new_cluster_detailed_entry)
            ...
            logger.debug(f"Fin de peuplement de l'ontologie. Données du profil après peuplement: {self.current_project_profile_data}")

            # Save configuration to persist the new clusters/labels
            self._save_configuration(show_message=False)

            self._load_profile_into_ui(self.current_project_name) # Refresh UI to show new clusters/labels
            ...
            QtWidgets.QMessageBox.information(
                ...
                tr("project_config.import_success_msg_multiple").format(directory_name=os.path.basename(selected_directory)) # New message for multiple clusters
            )
            logger.info(f"Structure du dossier '{selected_directory}' importée avec succès, créant plusieurs clusters.")

        except Exception as e:
            logger.error(f"Erreur lors de l'importation de la structure du dossier : {str(e)}")
    ...
    def _populate_ontology_from_directory(self, current_path, target_list, current_ontology_level):
        """
        ...
        try:
            ...
            logger.debug(f"Parcours de : {current_path} au niveau : {current_ontology_level}")
            for item_name in sorted(os.listdir(current_path)): # Tri pour un ordre cohérent
                ...
                if os.path.isdir(item_full_path):
                    ...
                    if current_ontology_level == "root":
                        ...
                        logger.debug(f"    Ajout d'un label racine (répertoire) : {item_name} (ID: {new_node['id']})")
                        self._populate_ontology_from_directory(item_full_path, new_node["parent_labels"], "parent")
                    elif current_ontology_level == "parent":
                        new_node["child_labels"] = []
                        target_list.append(new_node)
                        logger.debug(f"    Ajout d'un label parent (répertoire) : {item_name} (ID: {new_node['id']})")
                        self._populate_ontology_from_directory(item_full_path, new_node["child_labels"], "child")
                        ...
                        target_list.append(new_node)
                        logger.debug(f"    Ajout d'un label enfant (répertoire - terminal) : {item_name} (ID: {new_node['id']})")
                    ...
                    target_list.append(new_node)
                    logger.debug(f"    Ajout d'un label (fichier - terminal) : {item_name} (ID: {new_node['id']})")

        except Exception as e:
            logger.error(f"Erreur lors du parcours du dossier '{current_path}' : {str(e)}")
    ...
    def _get_selected_label_data(self, level):
        ...
        elif level == "parent" and self.current_root_label_index != -1 and self.current_parent_label_index != -1:
            if 0 <= self.current_root_label_index < len(root_labels_data):
        ...
        elif level == "child" and self.current_root_label_index != -1 and self.current_parent_label_index != -1 and self.current_child_label_index != -1:
            if 0 <= self.current_root_label_index < len(root_labels_data):
                selected_root = root_labels_data[self.current_root_label_index]
                parent_labels_data = selected_root.get('parent_labels', [])
                if 0 <= self.current_parent_label_index < len(parent_labels_data):
    ...
    def _update_label_item_text(self, list_widget, label_data):
        ...
        if not found_item:
            logger.warning(f"Impossible de trouver l'élément '{current_name}' dans la liste pour la mise à jour.")
            return


        display_name = label_data.get('name', '')
        # MODIFICATION: Afficher les catégories jointes par des virgules
        if label_data.get('type') == 'file' and label_data.get('category') and len(label_data['category']) > 0:
    ...
    def _on_root_label_selected(self, current, previous):
        ...
        logger.debug(f"Label Racine sélectionné. Index: {self.current_root_label_index}")
    ...
    def _update_parent_list(self):
        ...
        if self.current_root_label_index == -1 or not self.current_cluster_data:
            logger.debug("Aucun label racine ou cluster sélectionné, effacement des listes parent/enfant.")
            self._update_button_states()
            return
        ...
        if 0 <= self.current_root_label_index < len(root_labels_data):
            selected_root = root_labels_data[self.current_root_label_index]
            parent_labels_data = selected_root.get('parent_labels', [])
            logger.debug(f"Labels Parents pour la racine '{selected_root.get('name', 'N/A')}' : {parent_labels_data}")
            for p_label in parent_labels_data:
                ...
                if p_label.get('type') == 'file' and p_label.get('category') and len(p_label['category']) > 0:
            ...
            if parent_labels_data:
                # Déconnecter temporairement le signal pour éviter le double déclenchement
                self.parent_list_widget.currentItemChanged.disconnect(self._on_parent_label_selected)
                self.parent_list_widget.setCurrentRow(0)
                self.parent_list_widget.currentItemChanged.connect(self._on_parent_label_selected)
                # Appeler explicitement si la sélection de 0 n'a pas changé l'index
                self._on_parent_label_selected(self.parent_list_widget.currentItem(), None)
            else:
                self.child_list_widget.clear()
                self.current_child_label_index = -1
    ...
    def _on_parent_label_selected(self, current, previous):
        ...
        logger.debug(f"Label Parent sélectionné. Index: {self.current_parent_label_index}")
    ...
    def _update_child_list(self):
        ...
        if self.current_root_label_index == -1 or self.current_parent_label_index == -1 or not self.current_cluster_data:
            logger.debug("Aucun label parent ou racine ou cluster sélectionné, effacement de la liste enfant.")
            self._update_button_states()
            return
        ...
        if 0 <= self.current_root_label_index < len(root_labels_data):
            ...
            if 0 <= self.current_parent_label_index < len(parent_labels_data):
                selected_parent = parent_labels_data[self.current_parent_label_index]
                child_labels_data = selected_parent.get('child_labels', [])
                logger.debug(f"Labels Enfants pour le parent '{selected_parent.get('name', 'N/A')}' : {child_labels_data}")
                for c_label in child_labels_data:
                    # MODIFICATION: Afficher la catégorie pour les fichiers dans le nom du label
                    display_name = c_label.get('name', '')
                    if c_label.get('type') == 'file' and c_label.get('category') and len(c_label['category']) > 0:
                        display_name = f"{display_name} ({', '.join(c_label['category'])})"
                    item = QtWidgets.QListWidgetItem(display_name)
                ...
                if child_labels_data:
                    # Déconnecter temporairement le signal pour éviter le double déclenchement
                    self.child_list_widget.currentItemChanged.disconnect(self._on_child_label_selected)
                    self.child_list_widget.setCurrentRow(0)
                    self.child_list_widget.currentItemChanged.connect(self._on_child_label_selected)
                    # Appeler explicitement si la sélection de 0 n'a pas changé l'index
                    self._on_child_label_selected(self.child_list_widget.currentItem(), None)
    ...
    def _modify_category_for_selected_label(self, level):
        ...
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            ...
            self._update_label_item_text(list_widget, label_data) # Met à jour le texte affiché
            logger.info(f"Catégories de '{label_name}' ({level}) mises à jour à : '{new_categories}'")
            self.save_button.setEnabled(True) # Activer le bouton de sauvegarde

    def _add_cluster(self):
        """Ajoute un nouveau cluster."""
        if not self.current_project_profile_data:
            QtWidgets.QMessageBox.warning(self, tr("project_config.no_project_selected_title"), tr("project_config.no_project_selected_msg_add_cluster"))
            return

        name, ok = QtWidgets.QInputDialog.getText(self, tr("project_config.add_cluster_dialog_title"), tr("project_config.add_cluster_dialog_text"))
        if ok and name:
            name = name.strip()
            if not name: return
            ...
            if any(c['name'] == name for c in turing_ontology.get('clusters_detailed', [])):
                QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"), tr("project_config.duplicate_cluster_msg"))
                return
            ...
            turing_ontology.setdefault('clusters_detailed', []).append(new_cluster_detailed_entry)
            ...
            self.cluster_list_widget.setCurrentText(name) # Sélectionner le nouveau cluster
    ...
    def _edit_cluster(self):
        ...
        new_name, ok = QtWidgets.QInputDialog.getText(self, tr("project_config.edit_cluster_dialog_title"), tr("project_config.edit_cluster_dialog_text"),
        ...
        if ok and new_name:
            ...
            if any(c['name'] == new_name for c in turing_ontology.get('clusters_detailed', []) if c is not self.current_cluster_data):
                QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"), tr("project_config.duplicate_cluster_msg"))
                return
            ...
            self._load_profile_into_ui(self.current_project_name) # Rafraîchir l'UI
            self.cluster_list_widget.setCurrentText(new_name) # Sélectionner le cluster renommé
    ...
    def _remove_cluster(self):
        ...
        reply = QtWidgets.QMessageBox.question(self, tr("project_config.remove_cluster_dialog_title"),
                                            tr("project_config.remove_cluster_dialog_text").format(cluster_name=cluster_name),
                                            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
        if reply == QtWidgets.QMessageBox.Yes:
            turing_ontology = self.current_project_profile_data['turing_ontology']
            ...
            self._load_profile_into_ui(self.current_project_name) # Rafraîchir l'UI pour refléter la suppression
    ...
    def _add_root_label(self):
        ...
        if not self.current_cluster_data:
            QtWidgets.QMessageBox.warning(self, tr("project_config.no_cluster_selected_title"), tr("project_config.no_cluster_selected_msg_add_root"))
            return

        name, ok = QtWidgets.QInputDialog.getText(self, tr("project_config.add_root_dialog_title"), tr("project_config.add_root_dialog_text"))
        if ok and name:
            name = name.strip()
            if not name: return
            ...
            if any(r['name'] == name for r in root_labels_data):
                QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"), tr("project_config.duplicate_root_msg"))
                return

            new_label_data = {"name": name, "type": "directory", "id": str(uuid.uuid4()), "full_path": "", "parent_labels": []} # Added ID and full_path
            root_labels_data.append(new_label_data)
            self.current_cluster_data['root_labels'] = root_labels_data # S'assurer que les données sont à jour
            ...
            logger.info(f"Label racine ajouté à cluster '{self.current_cluster_data['name']}' : {name}")
            self._update_button_states()


    def _edit_root_label(self):
        """Modifie le label racine sélectionné dans le cluster actuel."""
        if not self.current_cluster_data: return # Protection supplémentaire
        ...
        new_name, ok = QtWidgets.QInputDialog.getText(self, tr("project_config.edit_root_dialog_title"), tr("project_config.edit_root_dialog_text"),
                                                        QtWidgets.QLineEdit.Normal, old_name)
        if ok and new_name:
            new_name = new_name.strip()
            if not new_name or new_name == old_name: return
            ...
            if any(r['name'] == new_name for r in root_labels_data if r is not label_data):
                QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"), tr("project_config.duplicate_root_msg"))
                return

            label_data['name'] = new_name
            self._update_label_item_text(self.root_list_widget, label_data) # Met à jour le texte affiché
            logger.info(f"Label racine renommé de {old_name} en {new_name} dans cluster '{self.current_cluster_data['name']}'")
            self.save_button.setEnabled(True)
    ...
    def _remove_root_label(self):
        ...
        reply = QtWidgets.QMessageBox.question(self, tr("project_config.remove_root_dialog_title"),
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            logger.info(f"Label racine supprimé de cluster '{self.current_cluster_data['name']}' : {root_name}")
            self.save_button.setEnabled(True)
            self._update_button_states()
    ...
    def _add_parent_label(self):
        ...
        if not self.current_cluster_data or self.current_root_label_index == -1:
            QtWidgets.QMessageBox.warning(self, tr("project_config.no_root_selected_title"), tr("project_config.no_root_selected_msg"))
            return

        name, ok = QtWidgets.QInputDialog.getText(self, tr("project_config.add_parent_dialog_title"), tr("project_config.add_parent_dialog_text"))
        if ok and name:
            name = name.strip()
            if not name: return

            selected_root = self.current_cluster_data['root_labels'][self.current_root_label_index]
            parent_labels_data = selected_root.get('parent_labels', [])

            if any(p['name'] == name for p in parent_labels_data):
                QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"), tr("project_config.duplicate_parent_msg"))
                return

            new_label_data = {"name": name, "type": "directory", "id": str(uuid.uuid4()), "full_path": "", "child_labels": []} # Added ID and full_path
            parent_labels_data.append(new_label_data)
            selected_root['parent_labels'] = parent_labels_data # S'assurer que les données sont à jour

            self._update_child_list() # Force le rafraîchissement
            self.parent_list_widget.setCurrentRow(self.parent_list_widget.count() - 1)
            logger.info(f"Label parent ajouté sous racine '{selected_root['name']}' : {name}")
            self._update_button_states()
            self.save_button.setEnabled(True)
    ...
    def _edit_parent_label(self):
        ...
        if not self.current_cluster_data or self.current_root_label_index == -1: return

        current_parent_row = self.parent_list_widget.currentRow()
        if current_parent_row == -1: return

        label_data = self._get_selected_label_data("parent")
        if not label_data: return

        old_name = label_data.get('name', '')
        new_name, ok = QtWidgets.QInputDialog.getText(self, tr("project_config.edit_parent_dialog_title"), tr("project_config.edit_parent_dialog_text"),
                                                        QtWidgets.QLineEdit.Normal, old_name)
        if ok and new_name:
            new_name = new_name.strip()
            if not new_name or new_name == old_name: return
            ...
            if any(p['name'] == new_name for p in parent_labels_data if p is not label_data):
                QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"), tr("project_config.duplicate_parent_msg"))
                return

            label_data['name'] = new_name
            self._update_label_item_text(self.parent_list_widget, label_data) # Met à jour le texte affiché
    ...
    def _remove_parent_label(self):
        ...
        reply = QtWidgets.QMessageBox.question(self, tr("project_config.remove_parent_dialog_title"),
                                            tr("project_config.remove_parent_dialog_text").format(parent_name=parent_name),
                                            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
        if reply == QtWidgets.QMessageBox.Yes:
            selected_root = self.current_cluster_data['root_labels'][self.current_root_label_index]
            parent_labels_data = selected_root.get('parent_labels', [])
    ...
    def _add_child_label(self):
        ...
        if not self.current_cluster_data or self.current_root_label_index == -1 or self.current_parent_label_index == -1:
            QtWidgets.QMessageBox.warning(self, tr("project_config.no_parent_selected_title"), tr("project_config.no_parent_selected_msg"))
            return

        name, ok = QtWidgets.QInputDialog.getText(self, tr("project_config.add_child_dialog_title"), tr("project_config.add_child_dialog_text"))
        if ok and name:
            name = name.strip()
            if not name: return

            selected_root = self.current_cluster_data['root_labels'][self.current_root_label_index]
            selected_parent = selected_root['parent_labels'][self.current_parent_label_index]
            child_labels_data = selected_parent.get('child_labels', [])

            if any(c['name'] == name for c in child_labels_data):
                QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"), tr("project_config.duplicate_child_msg"))
                return

            new_label_data = {"name": name, "type": "directory", "id": str(uuid.uuid4()), "full_path": ""} # Added ID and full_path
            child_labels_data.append(new_label_data)
            selected_parent['child_labels'] = child_labels_data # S'assurer que les données sont à jour
    ...
    def _edit_child_label(self):
        ...
        if not self.current_cluster_data or self.current_root_label_index == -1 or self.current_parent_label_index == -1: return

        current_child_row = self.child_list_widget.currentRow()
        if current_child_row == -1: return

        label_data = self._get_selected_label_data("child")
        if not label_data: return

        old_name = label_data.get('name', '')
        new_name, ok = QtWidgets.QInputDialog.getText(self, tr("project_config.edit_child_dialog_title"), tr("project_config.edit_child_dialog_text"),
                                                        QtWidgets.QLineEdit.Normal, old_name)
        if ok and new_name:
            new_name = new_name.strip()
            if not new_name or new_name == old_name: return
            ...
            if any(c['name'] == new_name for c in child_labels_data if c is not label_data):
                QtWidgets.QMessageBox.warning(self, tr("project_config.duplicate_title"), tr("project_config.duplicate_child_msg"))
                return

            label_data['name'] = new_name
            self._update_label_item_text(self.child_list_widget, label_data) # Met à jour le texte affiché
    ...
    def _remove_child_label(self):
        ...
        reply = QtWidgets.QMessageBox.question(self, tr("project_config.remove_child_dialog_title"),
    ...
    def _on_add_new_project(self):
        ...
        new_project_name, ok = QtWidgets.QInputDialog.getText(
            self, tr("project_config.new_project_dialog_title"), tr("project_config.new_project_dialog_text")
        )
        if ok and new_project_name:
            new_project_name = new_project_name.strip()
            if not new_project_name:
                QtWidgets.QMessageBox.warning(self, tr("project_config.invalid_name_title"), tr("project_config.invalid_name_msg"))
                return
            ...
            if self.database and self.database.get_project_profile(new_project_name):
                QtWidgets.QMessageBox.warning(
                    self, tr("project_config.project_exists_title"), tr("project_config.project_exists_msg").format(project_name=new_project_name)
                )
                return
            ...
            self.project_combo.setCurrentText(new_project_name) # Sélectionner le nouveau projet
            ...
            self.save_button.setEnabled(True)
            self.delete_project_button.setEnabled(True)
            self.export_dgraph_query_button.setEnabled(False) # Nouveau projet, pas encore de données à exporter
    ...
    def _on_delete_project(self):
        ...
        if not self.current_project_name:
            QtWidgets.QMessageBox.warning(self, tr("project_config.no_project_selected_title"), tr("project_config.no_project_selected_msg_delete"))
            return

        reply = QtWidgets.QMessageBox.question(
            self, tr("project_config.delete_project_dialog_title"),
            tr("project_config.delete_project_dialog_text").format(project_name=self.current_project_name),
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            try:
                if self.database:
                    success = self.database.delete_project_profile(self.current_project_name)
                    if not success:
                        raise Exception(f"Échec de la suppression du projet '{self.current_project_name}' de la base de données.")
                    logger.info(f"Projet '{self.current_project_name}' supprimé de la base de données.")
                else:
                    raise Exception("Base de données non disponible pour la suppression du projet.")
                ...
                logger.info(f"Projet '{self.current_project_name}' supprimé avec succès.")

                self.current_project_name = None
                self.current_project_profile_data = None
                self._update_project_combo() # Actualiser la boîte combinée
                self._reset_ui() # Effacer les champs de l'interface utilisateur
                QtWidgets.QMessageBox.information(self, tr("project_config.project_deleted_title"), tr("project_config.project_deleted_msg"))

            except Exception as e:
                logger.error(f"Erreur lors de la suppression du projet '{self.current_project_name}' : {str(e)}")
                QtWidgets.QMessageBox.critical(
                    self, tr("project_config.deletion_error_title"),
                    tr("project_config.deletion_error_msg").format(project_name=self.current_project_name, error=str(e))
                )
    ...
    def _save_configuration(self, show_message=True):
        ...
        if not self.current_project_profile_data:
            if show_message:
                QtWidgets.QMessageBox.warning(self, tr("project_config.no_project_selected_title"), tr("project_config.no_project_selected_msg_save"))
            return

        project_name = self.project_name_edit.text().strip()
        if not project_name:
            if show_message:
                QtWidgets.QMessageBox.warning(self, tr("project_config.missing_project_name_title"), tr("project_config.missing_project_name_msg"))
            return

        # Gérer le cas de renommage
        if self.current_project_name and project_name != self.current_project_name:
             reply = QtWidgets.QMessageBox.question(
                 self, tr("project_config.rename_project_dialog_title"),
                 tr("project_config.rename_project_dialog_text").format(old_name=self.current_project_name, new_name=project_name),
                 QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No
             )
             if reply == QtWidgets.QMessageBox.Yes:
                 if self.database:
                     ...
                     logger.info(f"Ancien projet '{self.current_project_name}' supprimé lors du renommage.")
                 
                 # Mettre à jour le nom du projet actuel après la suppression réussie de l'ancien
                 self.current_project_name = project_name
                 self.current_project_profile_data['name'] = project_name # Mettre à jour le nom dans les données également
             else:
                 return # L'utilisateur a annulé le renommage

        try:
            if not self.database:
                raise Exception("Base de données non disponible pour la sauvegarde du projet.")

            success = self.database.save_project_profile(project_name, self.current_project_profile_data)
            ...
            if success:
                ...
                self.project_profile_saved.emit(project_name, self.current_project_profile_data)
                ...
                if show_message:
                    QtWidgets.QMessageBox.information(
                        self, tr("project_config.config_saved_title"),
                        tr("project_config.config_saved_msg").format(project_name=project_name)
                    )
                logger.info(f"Profil de projet '{project_name}' enregistré avec succès.")
            else:
                if show_message:
                    QtWidgets.QMessageBox.critical(
                        self, tr("project_config.save_error_title"),
                        tr("project_config.save_error_msg").format(project_name=project_name)
                    )
                logger.error(f"Échec de l'enregistrement du profil de projet '{project_name}'.")

        except Exception as e:
            logger.error(f"Erreur lors de l'enregistrement de la configuration du projet pour '{project_name}' : {str(e)}")
            if show_message:
                QtWidgets.QMessageBox.critical(
                    self, tr("project_config.error_title"),
    ...
    def _load_project_profiles(self):
        ...
        try:
            if not self.database:
                logger.error("Base de données non disponible. Impossible de charger les profils de projet.")
                self.project_profiles = {}
                self._update_project_combo()
                self._reset_ui()
                return
            ...
            logger.info(f"Chargement de {len(self.project_profiles)} profils de projet depuis la base de données.")
            ...
            for project_name, profile in self.project_profiles.items():
                ...
                if 'cluster' in turing_ontology and turing_ontology['cluster'] and not turing_ontology['clusters_detailed']:
                    logger.warning(f"Migrating old 'cluster' list to 'clusters_detailed' for project '{project_name}'.")
                    for old_cluster_name in turing_ontology['cluster']:
                        turing_ontology['clusters_detailed'].append({"name": old_cluster_name, "id": str(uuid.uuid4()), "root_labels": []}) # Assign ID during migration
                elif 'cluster' not in turing_ontology: # Si 'cluster' n'existe pas du tout
                     turing_ontology['cluster'] = []

                # Migrer l'ancien format 'root_labels' s'il était au top-level (avant clusters_detailed)
                if 'root_labels' in turing_ontology and turing_ontology['root_labels'] and not turing_ontology['clusters_detailed']:
                    logger.warning(f"Migrating top-level 'root_labels' to a default cluster in 'clusters_detailed' for project '{project_name}'.")
                    default_cluster_name = tr("project_config.default_imported_cluster_name") # "Imported_Data"
                    if not turing_ontology['clusters_detailed']: # Si pas encore de clusters_detailed
                        turing_ontology['clusters_detailed'].append({
                            "name": default_cluster_name,
                            "id": str(uuid.uuid4()), # Assign ID during migration
                            "root_labels": turing_ontology.pop('root_labels') # Déplacer les labels racines existants
                        })
                        ...
                        if turing_ontology['clusters_detailed'][0]['name'] == default_cluster_name:
                            turing_ontology['clusters_detailed'][0]['root_labels'].extend(turing_ontology.pop('root_labels'))
                        else:
                            turing_ontology['clusters_detailed'].append({
                                "name": default_cluster_name,
                                "id": str(uuid.uuid4()), # Assign ID during migration
                            ...
                            if default_cluster_name not in turing_ontology['cluster']:
                                turing_ontology['cluster'].append(default_cluster_name)
                elif 'root_labels' in turing_ontology and not turing_ontology['root_labels']: # S'il est vide, le laisser tel quel ou le supprimer
                    pass # Laisser vide, la nouvelle logique gérera ça

                # S'assurer que 'cluster' contient les noms de 'clusters_detailed'
                turing_ontology['cluster'] = [c['name'] for c in turing_ontology['clusters_detailed']]
                ...
                def migrate_label_types(labels_list):
                    for label in labels_list:
                        ...
                        if 'full_path' not in label: # Assign full_path if missing
                            label['full_path'] = "" # Default empty, should be set on import
                        
                        if 'type' not in label:
                            # Inférence du type basée sur la présence de sous-labels
                            label['type'] = 'directory' if 'parent_labels' in label or 'child_labels' in label else 'file'
                        ...
                        if label['type'] == 'file':
                            ...
                            if 'category' in label: # Supprimer la catégorie si c'est un répertoire (ancienne donnée)
                                del label['category']
            ...
            if self.project_profiles:
                ...
                self.current_project_profile_data = self.project_profiles[first_project_name].copy()
            ...
            logger.error(f"Erreur critique lors du chargement de tous les profils de projet : {str(e)}")
            QtWidgets.QMessageBox.critical(
                self, tr("project_config.loading_error_title"),
                tr("project_config.generic_error_msg").format(error=str(e))
            )
    ...
    def _on_export_dgraph_query(self):
        ...
        """
        if not self.current_project_profile_data:
            QtWidgets.QMessageBox.warning(self, tr("project_config.export_dgraph_query_title"), tr("project_config.no_project_data_export_msg"))
            return
        
        if not self.is_configured():
            QtWidgets.QMessageBox.warning(self, tr("project_config.export_dgraph_query_title"), tr("project_config.not_configured_export_msg"))
            return

        project_name_slug = self.current_project_name.replace(" ", "_").replace("/", "_").replace("\\", "_")
        ...
        try:
            generated_code = self._generate_dgraph_import_script(self.current_project_profile_data)
            ...
            QtWidgets.QMessageBox.information(
                ...
                tr("project_config.export_dgraph_success_msg").format(file_path=file_path)
            )
            logger.info(f"Requête Dgraph exportée avec succès vers : {file_path}")

        except Exception as e:
            logger.error(f"Erreur lors de l'exportation de la requête Dgraph : {str(e)}")
            QtWidgets.QMessageBox.critical(
                self,
                tr("project_config.export_dgraph_query_title"),
...
"""
        ...
        final_script_content = script_content.replace("    # PROJECT_DATA_PLACEHOLDER", project_data_formatted)
