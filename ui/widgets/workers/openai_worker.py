...
class OpenAIWorker(QThread):
    """Worker pour effectuer des requêtes à l'API OpenAI"""
    ...
    def __init__(self, context, perimeter_data, api_key=None, model="gpt-4o"):
        super().__init__()
        self.context = context
        self.perimeter_data = perimeter_data
        self.api_key = api_key
        self.model = model  # gpt-4o, gpt-4o-mini, gpt-4-turbo, etc.
        self.platform_name = "OpenAI"
        self.platform_index = 2
        
    def run(self):
        """Exécute la requête OpenAI"""
        start_time = time.time()
        
        try:
            # Étape 1: Configuration de l'API
            self.step_update.emit("config", "Configuration de l'API OpenAI...")
            
            if not self.api_key:
                raise ValueError("Clé API OpenAI non configurée")
            
            # Utiliser le client OpenAI
            ...
            self.step_update.emit("request", f"Envoi de la requête à OpenAI ({self.model})...")
            
            # Créer la completion avec le modèle sélectionné
            response = client.chat.completions.create(
                model=self.model,
                messages=[
                    {
                        "role": "system",
                        "content": "Tu es un assistant expert en développement logiciel. Tu génères du code propre, bien documenté et suivant les meilleures pratiques."
                    },
                    {
                        "role": "user",
                        "content": prompt
            ...
            if not response_text:
                raise ValueError("Aucun contenu dans la réponse de OpenAI")
            ...
            # Informations sur le modèle et la fin de génération
            ...
            logger.info(f"OpenAI request completed successfully in {duration:.2f}s using {self.model}")
