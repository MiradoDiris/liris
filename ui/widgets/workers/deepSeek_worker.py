...
class DeepSeekWorker(QThread):
    """Worker pour effectuer des requêtes à l'API DeepSeek"""
    ...
    def __init__(self, context, perimeter_data, api_key=None, model="deepseek-chat"):
        super().__init__()
        self.context = context
        self.perimeter_data = perimeter_data
        self.api_key = api_key
        self.model = model  # deepseek-chat (V3.2), deepseek-reasoner (R1)
        self.platform_name = "DeepSeek"
        self.platform_index = 4
        
    def run(self):
        """Exécute la requête DeepSeek"""
        start_time = time.time()
        
        try:
            # Étape 1: Configuration de l'API
            self.step_update.emit("config", "Configuration de l'API DeepSeek...")
            
            if not self.api_key:
                raise ValueError("Clé API DeepSeek non configurée")
            
            # Utiliser le client OpenAI avec base_url DeepSeek
            # L'API DeepSeek est compatible avec l'API OpenAI
            client = OpenAI(
            ...
            model_display = "DeepSeek-V3.2" if self.model == "deepseek-chat" else "DeepSeek-R1"
            self.step_update.emit("request", f"Envoi de la requête à {model_display}...")
            
            # Créer la completion avec le modèle DeepSeek
            response = client.chat.completions.create(
                model=self.model,
                messages=[
                    {
                        "role": "system",
                        "content": "Tu es un assistant IA expert en développement logiciel. Tu génères du code propre, efficace et bien documenté en suivant les meilleures pratiques."
                    },
                    {
                        "role": "user",
                        "content": prompt
            ...
            if not response_text:
                raise ValueError("Aucun contenu dans la réponse de DeepSeek")
            ...
            if usage:
                ...
                # Calculer le coût approximatif (tarifs DeepSeek très compétitifs)
                # deepseek-chat: Input $0.27/1M tokens, Output $1.10/1M tokens
                ...
                if self.model == "deepseek-chat":
                    ...
                    output_cost = (usage.completion_tokens / 1_000_000) * 2.19
                else:
            ...
            if finish_reason == "length":
                ...
                self.debug_info.emit("Avertissement: Contenu filtré par DeepSeek")
            
            # Pour deepseek-reasoner, logger si des pensées de raisonnement sont présentes
            if self.model == "deepseek-reasoner" and hasattr(response.choices[0].message, 'reasoning_content'):
                self.debug_info.emit("Mode raisonnement activé - processus de pensée détaillé disponible")
            ...
            logger.info(f"DeepSeek request completed successfully in {duration:.2f}s using {self.model}")
            
        except Exception as e:
            duration = time.time() - start_time
            error_msg = f"Erreur DeepSeek: {str(e)}"
...
8. **Performance**: Optimisez le code pour l'efficacité et la maintenabilité
