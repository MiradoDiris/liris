...
class ProjectConfigOnlyWidget(QtWidgets.QWidget):
    ...
    def __init__(self, config_provider, conductor, parent=None):
        ...
        print("ProjectConfigOnlyWidget: Initialisation (onglet ProjectConfigWidget et NodeCreationWidget)...")

        self.config_provider = config_provider
        self.conductor = conductor
        self.project_config_widget_instance = None
        self.node_creation_widget_instance = None # <-- Nouvelle instance pour le nouvel onglet

        try:
            self._init_ui()
            print("ProjectConfigOnlyWidget: Initialisation terminée avec succès.")
        except Exception as e:
            logger.error(f"Erreur lors de l'initialisation de ProjectConfigOnlyWidget: {str(e)}")
    ...
    def _init_ui(self):
        ...
        self.tabs.addTab(self.node_creation_widget_instance, "Création de Nœuds") # <-- NOUVEL ONGLET
    ...
    def _force_enable_all_widgets(self):
        if self.project_config_widget_instance:
            ...
            for child in self.project_config_widget_instance.findChildren(QtWidgets.QWidget):
        ...
        if self.node_creation_widget_instance:
            ...
            for child in self.node_creation_widget_instance.findChildren(QtWidgets.QWidget):
                child.setEnabled(True)
    ...
    def refresh(self):
        ...
        if self.project_config_widget_instance and hasattr(self.project_config_widget_instance, 'refresh'):
            self.project_config_widget_instance.refresh()
        # NOUVEAU: Rafraîchir le NodeCreationWidget
        if self.node_creation_widget_instance and hasattr(self.node_creation_widget_instance, 'refresh'):
